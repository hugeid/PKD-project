...
import Game
    ( cellSize,
      ...
      Button(..),
      Cell(..),
      Game(Game, bs, state, board, player),
...
--uncomment this line to use circles instead of hexagons: hexagon x y c = translate x y $ color c $ circleSolid (cellWidth/2)

{- hexaCorners x y s
    calculates the corners for a hexagon
...
{- gameTurn p
  ...
  RETURNS: A picture containing "Player to move:" text and a circle of color p's color
  EXAMPLES: gameTurn initialGame {player = Player red}   == Translate 85.0 300.0 (Pictures [Scale 0.2 0.2 (Text "Player to move:"),
                                                     Translate 1.0 0.0 (Color grey (Scale 0.2 0.2 (Text "Player to move:"))),
                                                     Translate 230.0 10.0 (Color red (ThickCircle 7.5 15.0))])
            gameTurn initialGame {player = Player blue}  == Translate 85.0 300.0 (Pictures [Scale 0.2 0.2 (Text "Player to move:"),
                                                     Translate 1.0 0.0 (Color grey (Scale 0.2 0.2 (Text "Player to move:"))),
